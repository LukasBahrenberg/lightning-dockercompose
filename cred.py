...
subprocess_result = subprocess.run('sudo docker exec lit litcli --network {} --lndtlscertpath /root/.lnd/tls.cert sessions add --label="default" --type=admin'.format(network), shell=True)